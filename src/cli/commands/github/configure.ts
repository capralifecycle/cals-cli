...
async function processProjects(
  ...
  for (const project of projects) {
    ...
    for (const projectRepo of project.repos) {
      ...
      const repo = await github.getRepository(org.login, projectRepo.name)
      if (repo === undefined) {
  ...
  const knownRepos = projects.flatMap(it => it.repos).map(it => it.name)
  const unknownRepos = (await github.getRepoList({ owner: org.login })).filter(
    it => !knownRepos.includes(it.name),
  )
...
const teamsCommand: CommandModule = {
  ...
  handler: async () => {
    ...
    await reportRateLimit(reporter, github, async () => {
      const org = await github.getOrg('capralifecycle')
      const definition = getDefinition(github)
      await processTeams(reporter, github, org, definition.teams)
    })
  },
}
