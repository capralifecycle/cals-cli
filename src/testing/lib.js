 ...
 */
export async function createNetwork(executor) {
 ...
 */
export async function curl(executor, network, ...args) {
    executor.checkCanContinue();
 ...
 */
export async function pollForCondition({ container, attempts, waitIntervalSec, condition, }) {
    function log(value) {
...
export async function waitForHttpOk({ container, url, attempts = 30, waitIntervalSec = 1, }) {
...
export async function waitForPostgresAvailable({ container, attempts = 30, waitIntervalSec = 1, username = "user", password = "password", dbname, }) {
    await pollForCondition({
        container,
        attempts,
        waitIntervalSec,
...
export async function startContainer({ executor, network, imageId, alias, env, dockerArgs = [], pull = false, }) {
    executor.checkCanContinue();
...
export async function runNpmRunScript(name, options) {
 ...
 */
export async function getDockerHostAddress() {
...
export async function waitForEnterToContinue(prompt = "Press enter to continue") {
